...
Deploy to staging, then run **Staging acceptance** with the exact build SHA, frontend URL and Supabase health URL. The protected `staging` Environment must provide the dedicated test-user credentials described in `docs/GITHUB_GO_LIVE_SETUP.md`; the workflow fails if they are absent and exercises authenticated buyer routes without committing a session or password.

After all external approvals are linked in the protected `production` Environment, run **Production candidate gate** with that exact accepted SHA and staging-acceptance run ID. The gate checks out the immutable accepted SHA, downloads and verifies its staging evidence, re-runs repository, test, security, native-policy and build contracts, compiles with production-only environment values, verifies the artifact manifest and SBOM, and emits `production-gate-evidence.json`. Zivvo's environment values are compiled by Vite, so production is a separate configuration build from the same accepted source SHA—not a byte-for-byte promotion of the staging bundle. Deploy only the emitted `zivvo-production-candidate-<SHA>` artifact with `public/_headers` and `public/_redirects`, or the equivalent hosting configuration.

After each external deployment, run **Post-deploy verification**. It creates a GitHub Deployment record and will only mark it successful after verifying the embedded SHA, release environment, critical routes, security headers and database-backed health check.
