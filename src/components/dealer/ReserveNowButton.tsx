  ...
  const submit = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("reserve-deposit", {
        ...
        body: { listing_id: listingId, dealer_id: dealerId, buyer_name: form.buyer_name, buyer_phone: form.buyer_phone },
      });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="grid gap-3 py-2">
          ...
          <div><Label>{t("dealer.reserveNow.depositAmount")}</Label><Input value="€500" readOnly aria-readonly="true" /></div>
        </div>
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={loading} className="border-0 text-white" style={accent ? { backgroundColor: accent } : undefined}>
            {loading ? t("dealer.reserveNow.redirecting") : t("dealer.reserveNow.payAndReserve", { amount: "€500" })}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
