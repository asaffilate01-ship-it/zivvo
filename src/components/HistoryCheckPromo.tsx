        ...
        <div className="grid items-center gap-10 lg:grid-cols-2">
          ...
          >
            ...
            <div className="absolute -bottom-4 -left-4 rounded-2xl border border-border bg-card p-4 shadow-elevated">
              ...
              <p className="text-xs text-muted-foreground">{t("home.hpiPromo.statLabel")}</p>
            </div>
          </motion.div>
          ...
          >
            <div className="inline-flex items-center gap-2 rounded-full border border-success/30 bg-success/10 px-3 py-1 text-xs font-medium text-success">
              ...
              {t("home.hpiPromo.badge")}
            </div>
            <h2 className="mt-3 font-display text-2xl font-bold text-foreground md:text-4xl">
              {t("home.hpiPromo.title")}
            </h2>
            <p className="mt-3 max-w-lg text-muted-foreground">
              {t("home.hpiPromo.subtitle")}
            </p>

            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {[
                { icon: FileSearch, label: t("home.hpiPromo.f1") },
                { icon: AlertTriangle, label: t("home.hpiPromo.f2") },
                { icon: CheckCircle2, label: t("home.hpiPromo.f3") },
                { icon: ShieldCheck, label: t("home.hpiPromo.f4") },
              ].map((f) => (
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              ...
              <Link to="/valuation">
                <Button size="lg" variant="outline" className="gap-1.5">
                  {t("home.hpiPromo.ctaAlt")}
                </Button>
              </Link>
            </div>
            <p className="mt-3 text-xs text-muted-foreground">
              {t("home.hpiPromo.poweredBy")}
            </p>
          </motion.div>
        </div>
      </div>
