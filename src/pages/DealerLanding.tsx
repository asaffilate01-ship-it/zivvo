...
import {
  ...
  Fuel, Settings2, Calendar, SlidersHorizontal,
} from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
        ...
        {heroStyle === "split" ? (
          <div className="grid lg:grid-cols-2">
            <div className="flex flex-col justify-center px-6 py-16 md:px-12 lg:py-24">
              <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }}>
                ...
                <h1 className="font-display text-3xl font-bold text-foreground md:text-5xl lg:text-6xl">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <a href="#inventory">
                    <Button size="lg" className="border-0 text-white" style={{ backgroundColor: accent }}>
                  ...
                  {showPhone && dealer.business_phone && (
                    <a href={`tel:${dealer.business_phone}`}>
                      <Button size="lg" variant="outline">
                        <Phone className="mr-1 h-4 w-4" /> Call Us
                      </Button>
                    </a>
                  )}
                </div>
          ...
          <>
            ...
            <div className="container relative mx-auto px-4 py-20 md:py-32">
              ...
              >
                {dealer.logo_url && (
                  <div className="mx-auto mb-6 inline-flex h-20 w-20 items-center justify-center rounded-2xl border border-white/10 bg-white/10 backdrop-blur-md">
                    <img src={dealer.logo_url} alt={dealer.business_name} className="h-14 w-14 rounded-xl object-contain" />
                  </div>
                )}
        ...
        <section className="border-b border-border bg-card">
          <div className="container mx-auto grid grid-cols-2 gap-px md:grid-cols-4">
            {[
              ...
              { icon: Shield, value: "100%", label: "Verified Dealer" },
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
        ...
        <section className="border-b border-border">
          <div className="container mx-auto grid gap-8 px-4 py-12 lg:grid-cols-2 lg:py-16">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <h2 className="font-display text-2xl font-bold text-foreground md:text-3xl">About {dealer.business_name}</h2>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="rounded-2xl border border-border bg-card p-6 shadow-card">
                <h3 className="font-display text-base font-semibold text-card-foreground mb-4">Contact Information</h3>
          ...
          {filteredListings.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-card py-20 text-center">
              ...
              <h3 className="mt-4 font-display text-lg font-semibold text-foreground">
                {searchQuery || filterFuel !== "all" || filterBody !== "all"
      ...
      {config.show_testimonials !== false && config.testimonials && config.testimonials.length > 0 && (
        <section className="border-t border-border bg-muted/30 py-12">
          <div className="container mx-auto px-4">
            <h2 className="font-display text-xl font-bold text-foreground text-center mb-8">What Our Customers Say</h2>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {config.testimonials.map((t, i) => (
                ...
                >
                  ...
                  <p className="mt-3 text-xs font-semibold text-card-foreground">— {t.name}</p>
                </motion.div>
              ))}
            </div>
          </div>
      ...
      {/* ─── CTA Footer ─── */}
      <section className="border-t border-border" style={{ background: `linear-gradient(135deg, ${accent}08, ${accent}15)` }}>
        <div className="container mx-auto px-4 py-12 text-center">
          <h2 className="font-display text-xl font-bold text-foreground md:text-2xl">
            Ready to find your next car?
          </h2>
          <p className="mt-2 text-muted-foreground">Contact us today or browse our full range</p>
