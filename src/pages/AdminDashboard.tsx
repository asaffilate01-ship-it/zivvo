...
const AdminDashboard = () => {
  ...
  const approveKYC = async (dealerId: string) => {
    const { error } = await supabase
      .from("dealers")
      .update({ kyc_verified: true, kyc_approved_at: new Date().toISOString() })
      .eq("id", dealerId);
    if (error) {
  ...
  const rejectKYC = async (dealerId: string) => {
    const { error } = await supabase
      .from("dealers")
      .update({ kyc_verified: false, is_active: false })
      .eq("id", dealerId);
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    } else {
      toast({ title: "Dealer rejected" });
      fetchAll();
    }
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { label: "Active Listings", value: activeListings.length, icon: Car, color: "text-accent-foreground" },
            { label: "Monthly Revenue", value: `$${totalRevenue.toLocaleString()}`, icon: DollarSign, color: "text-primary" },
        ...
        <Tabs defaultValue="dealers" className="mt-8">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => exportCSV(dealers, "dealers")}>
          ...
          <TabsContent value="listings" className="mt-4">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button variant="outline" size="sm" onClick={() => exportCSV(listings, "listings")}>
                  <Download className="mr-1 h-4 w-4" />
                  CSV
                </Button>
              </CardHeader>
                      ...
                      <TableRow key={l.id}>
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            {l.status !== "active" && (
                              <Button size="sm" variant="ghost" onClick={() => toggleListingStatus(l.id, "active")}>
                                Approve
                              </Button>
                            )}
                            {l.status === "active" && (
                              <Button size="sm" variant="ghost" onClick={() => toggleListingStatus(l.id, "under_review")}>
                                Suspend
                              </Button>
                            )}
                          </div>
          ...
          <TabsContent value="finance" className="mt-4">
            ...
            <div className="mt-4 flex gap-2">
              <Button variant="outline" onClick={() => exportCSV(dealers, "dealers-finance")}>
                <FileText className="mr-1 h-4 w-4" />
                Export Dealers CSV
              </Button>
              <Button variant="outline" onClick={() => exportCSV(commissions, "commissions")}>
                <FileText className="mr-1 h-4 w-4" />
