...
const statusColors: Record<string, string> = {
  ...
  seller_accepted: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200",
  seller_rejected: "bg-destructive/20 text-destructive",
  acquired: "bg-primary/20 text-primary",
  listed_to_dealers: "bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200",
  dealer_accepted: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200",
  seller_paid: "bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200",
...
const TradeStock = () => {
  ...
  const createDeal = useMutation({
    mutationFn: async () => {
      if (!selectedListing || !sellerPrice) throw new Error(t("tradeStock.errors.selectListingAndPrice"));
      const listing = listings.find((l: any) => l.id === selectedListing);
      if (!listing) throw new Error(t("tradeStock.errors.listingNotFound"));
    ...
    onSuccess: () => {
      toast.success(t("tradeStock.toasts.dealCreated"));
      setShowCreate(false);
      setSelectedListing("");
      setSellerPrice("");
  ...
  return (
    <>
      <SEOHead title={t("tradeStock.seo.title")} description={t("tradeStock.seo.description")} />
      <Navbar />
        ...
        <section className="text-white py-16 md:py-20 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 relative z-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <Badge className="bg-primary/20 text-primary border-primary/30 hover:bg-primary/30">{t("tradeStock.hero.badge")}</Badge>
            </div>
            <h1 className="text-3xl md:text-5xl font-bold font-display mb-3 tracking-tight">
              {t("tradeStock.hero.title1")}<br className="hidden md:block" /> {t("tradeStock.hero.title2")}
            </h1>
            <p className="text-white/70 max-w-2xl text-base md:text-lg leading-relaxed">
              {t("tradeStock.hero.subtitle")}
            </p>
          </div>
        </section>
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-6">
            <div className="relative flex-1">
              ...
              <Input placeholder={t("tradeStock.search.placeholder")} value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10" />
            </div>
            {isAdmin && (
              <Dialog open={showCreate} onOpenChange={setShowCreate}>
                <DialogTrigger asChild>
                  <Button className="gap-2"><Plus className="w-4 h-4" /> {t("tradeStock.createDeal.button")}</Button>
                </DialogTrigger>
                <DialogContent className="max-w-lg">
                  <DialogHeader>
                    <DialogTitle>{t("tradeStock.createDeal.dialogTitle")}</DialogTitle>
                    <DialogDescription>{t("tradeStock.createDeal.dialogDesc")}</DialogDescription>
                  </DialogHeader>
                  <div className="space-y-4 py-2">
                    <div>
                      <Label>{t("tradeStock.createDeal.selectListing")}</Label>
                      <Select value={selectedListing} onValueChange={setSelectedListing}>
                        <SelectTrigger><SelectValue placeholder={t("tradeStock.createDeal.chooseCar")} /></SelectTrigger>
                        <SelectContent>
                          {listings.map((l: any) => (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label>{t("tradeStock.createDeal.buyFromSellerAt")}</Label>
                        <Input type="number" value={sellerPrice} onChange={(e) => setSellerPrice(e.target.value)} placeholder={t("tradeStock.createDeal.sellerPricePlaceholder")} />
                      </div>
                      <div>
                        <Label>{t("tradeStock.createDeal.markupPct")}</Label>
                    ...
                    {sellerPrice && (
                      <Card className="bg-muted/50">
                        <CardContent className="p-4 space-y-2 text-sm">
                          <div className="flex justify-between"><span className="text-muted-foreground">{t("tradeStock.createDeal.sellerPriceLabel")}</span><span>{fmt(parseFloat(sellerPrice), country)}</span></div>
                          <div className="flex justify-between"><span className="text-muted-foreground">{t("tradeStock.createDeal.platformMarkup", { pct: markupPct })}</span><span className="text-primary font-medium">{fmt(parseFloat(platformMarkup || "0"), country)}</span></div>
                          <Separator />
                          <div className="flex justify-between font-bold"><span>{t("tradeStock.createDeal.dealerPays")}</span><span>{fmt(parseFloat(dealerPrice || "0"), country)}</span></div>
                        </CardContent>
                      </Card>
                    )}
                    <div>
                      <Label>{t("tradeStock.createDeal.adminNotes")}</Label>
                      <Textarea value={adminNotes} onChange={(e) => setAdminNotes(e.target.value)} placeholder={t("tradeStock.createDeal.adminNotesPlaceholder")} rows={2} />
                    </div>
                  </div>
                  <DialogFooter>
                    <Button onClick={() => createDeal.mutate()} disabled={createDeal.isPending}>
                      {createDeal.isPending ? t("tradeStock.createDeal.creating") : t("tradeStock.createDeal.create")}
                    </Button>
                  </DialogFooter>
                </DialogContent>
              </Dialog>
          ...
          <Tabs value={tab} onValueChange={setTab}>
            <TabsList className="flex-wrap">
              <TabsTrigger value="available" className="gap-1"><Building2 className="w-4 h-4" /> {t("tradeStock.tabs.available")}</TabsTrigger>
              <TabsTrigger value="my_offers" className="gap-1"><Receipt className="w-4 h-4" /> {t("tradeStock.tabs.myOffers")}</TabsTrigger>
              <TabsTrigger value="my_deals" className="gap-1"><CheckCircle2 className="w-4 h-4" /> {t("tradeStock.tabs.myDeals")}</TabsTrigger>
              {isAdmin && <TabsTrigger value="all" className="gap-1"><Eye className="w-4 h-4" /> {t("tradeStock.tabs.all")}</TabsTrigger>}
              {isAdmin && <TabsTrigger value="audit" className="gap-1"><FileText className="w-4 h-4" /> {t("tradeStock.tabs.audit")}</TabsTrigger>}
            </TabsList>
            ...
            <TabsContent value={tab === "my_offers" ? "__skip__" : tab} className={tab === "my_offers" ? "hidden" : "mt-6"}>
              {tab === "audit" ? (
                  ...
                  <CardContent className="p-4">
                    <ScrollArea className="h-96">
                      <div className="space-y-2">
                        ...
                        {auditLog.length === 0 && <p className="text-center text-muted-foreground py-6">{t("tradeStock.audit.empty")}</p>}
                      </div>
                    </ScrollArea>
                  </CardContent>
                </Card>
                ...
                <div className="text-center py-16">
                  ...
                  <h3 className="text-lg font-semibold mb-1">{t("tradeStock.empty.title")}</h3>
                  <p className="text-muted-foreground">{t("tradeStock.empty.subtitle")}</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        <CardContent className="p-4 space-y-3">
                          <div>
                            ...
                            <div className="flex items-center gap-2 text-xs text-muted-foreground mt-1">
                              {listing?.mileage && <span>{listing.mileage.toLocaleString()} {t("tradeStock.dealCard.mi")}</span>}
                          ...
                          {isAdmin && (
                            <div className="text-xs space-y-1 p-2 rounded-lg bg-muted/50">
                              <div className="flex justify-between"><span className="text-muted-foreground">{t("tradeStock.dealCard.sellerPrice")}</span><span>{fmt(deal.seller_price, deal.country)}</span></div>
                              <div className="flex justify-between"><span className="text-muted-foreground">{t("tradeStock.dealCard.markup", { pct: deal.markup_pct })}</span><span className="text-primary font-medium">+{fmt(deal.platform_markup, deal.country)}</span></div>
                              <div className="flex justify-between font-medium"><span>{t("tradeStock.dealCard.dealerPrice")}</span><span>{fmt(deal.dealer_price, deal.country)}</span></div>
                            </div>
                          )}
                          ...
                          {showDealerActions && deal.status === "listed_to_dealers" && (
                            <Button className="w-full gap-2" onClick={() => acceptDeal.mutate(deal.id)} disabled={acceptDeal.isPending}>
                              <CheckCircle2 className="w-4 h-4" /> {acceptDeal.isPending ? t("tradeStock.dealCard.accepting") : t("tradeStock.dealCard.acceptAndPurchase")}
                            </Button>
                          )}
                          ...
                          {showAdminActions && (
                            <div className="flex flex-wrap gap-2">
                              ...
                              {deal.seller_payment_ref && (
                                <span className="text-[10px] text-muted-foreground self-center">{t("tradeStock.dealCard.ref", { ref: deal.seller_payment_ref })}</span>
                              )}
          ...
          <Dialog open={!!payoutDialog} onOpenChange={() => { setPayoutDialog(null); setPayoutRef(""); }}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{t("tradeStock.payoutDialog.title")}</DialogTitle>
                <DialogDescription>
                  {t("tradeStock.payoutDialog.description", { vehicle: payoutDialog?.car_listings?.title || t("tradeStock.payoutDialog.defaultVehicle"), amount: payoutDialog ? fmt(payoutDialog.seller_price, payoutDialog.country) : "" })}
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-3 py-2">
                <div>
                  <Label>{t("tradeStock.payoutDialog.paymentRefLabel")}</Label>
                  <Input
                    value={payoutRef}
                    onChange={(e) => setPayoutRef(e.target.value)}
                    placeholder={t("tradeStock.payoutDialog.paymentRefPlaceholder")}
                  />
                </div>
                <div className="bg-muted/50 rounded-lg p-3 text-sm space-y-1">
                  <div className="flex justify-between"><span className="text-muted-foreground">{t("tradeStock.payoutDialog.sellerPrice")}</span><span>{payoutDialog ? fmt(payoutDialog.seller_price, payoutDialog.country) : ""}</span></div>
                  <div className="flex justify-between"><span className="text-muted-foreground">{t("tradeStock.payoutDialog.platformRevenue")}</span><span className="text-primary">{payoutDialog ? fmt(payoutDialog.platform_markup, payoutDialog.country) : ""}</span></div>
                </div>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setPayoutDialog(null)}>{t("tradeStock.payoutDialog.cancel")}</Button>
                <Button
                ...
                >
                  ...
                  {markSellerPaid.isPending ? t("tradeStock.payoutDialog.processing") : t("tradeStock.payoutDialog.confirm")}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
          ...
          <section className="mt-16 pb-8">
            <div className="text-center mb-10">
              <Badge variant="outline" className="mb-3">{t("tradeStock.howItWorks.badge")}</Badge>
              <h2 className="text-2xl md:text-3xl font-bold font-display">{t("tradeStock.howItWorks.title")}</h2>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
              {[
                { icon: Search, step: "01", title: t("tradeStock.howItWorks.step1Title"), desc: t("tradeStock.howItWorks.step1Desc") },
                { icon: DollarSign, step: "02", title: t("tradeStock.howItWorks.step2Title"), desc: t("tradeStock.howItWorks.step2Desc") },
                { icon: Shield, step: "03", title: t("tradeStock.howItWorks.step3Title"), desc: t("tradeStock.howItWorks.step3Desc") },
                { icon: Building2, step: "04", title: t("tradeStock.howItWorks.step4Title"), desc: t("tradeStock.howItWorks.step4Desc") },
                { icon: Truck, step: "05", title: t("tradeStock.howItWorks.step5Title"), desc: t("tradeStock.howItWorks.step5Desc") },
                ...
                <div key={step} className="relative text-center group">
                  ...
                  <p className="text-[10px] font-mono text-primary font-semibold tracking-widest mb-1">{t("tradeStock.howItWorks.step", { n: step })}</p>
