...
serve(async (req) => {
  ...
  try {
    const { make, model, year, mileage, fuel_type, transmission, body_type, color, engine_size, price } = await req.json();
    const LOVABLE_API_KEY = Deno.env.get("LOVABLE_API_KEY");
    if (!LOVABLE_API_KEY) throw new Error("LOVABLE_API_KEY is not configured");

    const prompt = `Write a compelling, professional car listing description for a ${year} ${make} ${model}. 
Details: ${[
...
Write 2-3 paragraphs. Be factual, highlight key selling points, mention comfort, performance, and value. Do NOT use markdown headings. Keep it under 200 words. Sound professional but warm.`;
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are an expert automotive copywriter. Write concise, compelling car listing descriptions." },
