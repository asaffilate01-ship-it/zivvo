...
Deno.serve(async (req) => {
  try {
    ...
    const { data: active } = await admin.from("reservation_deposits").select("id").eq("listing_id", listingId).in("status", ["pending", "paid", "held"]).gt("expires_at", new Date().toISOString()).maybeSingle();
